...
describe('calculateHierarchicalMetrics hierarchy', () => {
    ...
    it('includes Total Portfolio as the first row', () => {
        ...
        db.prepare("INSERT INTO holdings_ledger (snapshot_date, account_id, ticker, quantity, asset_type, market_value) VALUES (?, 'acc1', 'VTI', 10, 'EQUITY', 2000)").run(TODAY_ANCHOR);
    ...
    it('calculates Other / Uncategorized for tickers not in Level 0 categories', () => {
        ...
        db.prepare("INSERT INTO holdings_ledger (snapshot_date, account_id, ticker, quantity, asset_type, market_value) VALUES (?, 'acc1', 'DOGE', 1000, 'EQUITY', 150)").run(TODAY_ANCHOR);
